"""Module providing a function to generate temperature files from SAMeT product CPTEC/INPE"""
import os
import gc
...
def process_cptec_temp_epiweek(main_dir, output_dir, folder_name, shapefile_path, variable_name, years, indicator_name = "temp", color_png_file=None, aggregation_type = "all", interval_file_path=None, provide_interval=False):
    """
    Process SAMeT/CPTEC temperature data by epidemiological week.
    ...
    list_climate_format_files(file_paths = main_dir, output_dir = indicator_dir, variable_input = variable_name, indicator_name_local = indic_name, years_work = years, extension_folder = "epiweek", type_indicator = "temp", source="cptec")
    ...
    move_indicators_shapefiles_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "epiweek", extension_spatial = "mun", dates_col = dates_shapefile_col, anomaly_data = False, aggregation = aggregation_type, data_source = "SAMeT (CPTEC/INPE)", source="cptec")
...
def process_cptec_temp_month(main_dir, output_dir, folder_name, shapefile_path, variable_name, years, indicator_name = "temp", color_png_file=None, aggregation_type = "all"):
    """
    Process SAMeT/CPTEC temperature data by month.
    ...
    # Create a set of cog images from NetCDF files - generate_COG.py
    list_climate_format_files(file_paths = main_dir, output_dir = indicator_dir, variable_input = variable_name, indicator_name_local = indic_name, years_work = years, extension_folder = "month", type_indicator = "temp", source="cptec")
    ...
    # Move shapefiles files to new folder - utils.py
    move_indicators_shapefiles_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "month", extension_spatial = "mun", dates_col = dates_shapefile_col, anomaly_data = False, aggregation=aggregation_type, data_source = "SAMeT (CPTEC/INPE)", source="cptec")
