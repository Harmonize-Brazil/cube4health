"""Module providing a function to generate precipitation files from MERGE product CPTEC/INPE."""
import os
import gc
...
from cube4health.eclimpr.extract_aggregations import extract_indicators_max_min_mean

# -----------
# Functions:
# -----------

def process_cptec_precip_epiweek(main_dir, output_dir, folder_name, shapefile_path, variable_name, years, indicator_name = "precip", color_png_file=None, aggregation_type = "all", interval_file_path=None, provide_interval=False):
    """
    Process MERGE/CPTEC precipitation data by epidemiological week.

    Parameters
    ----------
    main_dir : str
        Directory path where the GRIB2 files are stored.
    output_dir : str
    ...
    variable_name : str
        Variable name in the GRIB2 file.
    years : list or str
    ...
    None
    """
    print("\n--- Starting processing MERGE/CPTEC precipitation by epidemiological week ...\n")
    ...
    list_climate_format_files(file_paths = main_dir, output_dir = indicator_dir, variable_input = variable_name, indicator_name_local = indic_name, years_work = years, extension_folder = "epiweek", type_indicator = "precip", source="cptec")
    ...
    move_indicators_cog_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "epiweek", extension_spatial = "mun", aggregation = aggregation_type, source="cptec")

    # Move shapefiles files to new folder - utils.py
    move_indicators_shapefiles_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "epiweek", extension_spatial = "mun", dates_col = dates_shapefile_col, anomaly_data = False, aggregation = aggregation_type, data_source = "MERGE (CPTEC/INPE)", source="cptec")
...
def process_cptec_precip_month(main_dir, output_dir, folder_name, shapefile_path, variable_name, years, indicator_name = "precip", color_png_file=None, aggregation_type = "all"):
    """
    Process MERGE/CPTEC precipitation data by month.
    ...
    None
    """
    print("\n--- Starting processing MERGE/CPTEC precipitation by month ...\n")
    ...
    # Create a set of cog images from NetCDF files - generate_COG.py
    list_climate_format_files(file_paths = main_dir, output_dir = indicator_dir, variable_input = variable_name, indicator_name_local = indic_name, years_work = years, extension_folder = "month", type_indicator = "precip", source="cptec")
    ...
    # Move COG files to new folder - utils.py
    move_indicators_cog_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "month", extension_spatial = "mun", aggregation=aggregation_type, source="cptec")

    # Move shapefiles files to new folder - utils.py
    move_indicators_shapefiles_files(main_dir = indicator_dir, output_final_dir = output_dir, indicator_name_local = [indic_name, folder_name], color_png_file = color_png_file, extension_folder = "month", extension_spatial = "mun", dates_col = dates_shapefile_col, anomaly_data = False, aggregation=aggregation_type, data_source = "MERGE (CPTEC/INPE)", source="cptec")
